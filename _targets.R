...
targets <- tarchetypes::tar_map(
  ...
  tar_target(samplesheet, epipe::read_file(samplesheet_path)),
  ...
  tar_target(betas_matrix, minfi::getBeta(clean_age)), #Extract betas without saving to disk
  ...
  # DMRS
  tar_target(dmrs,epipe::find_dmrs(beta_values=betas_matrix,model=model,fdr = fdrDMR,bcutoff = 0.05, min.cpg=min.cpgDMR,
                                   arraytype=arraytype,pal=pal_discrete),deployment = "worker"),
