...
targets <- tarchetypes::tar_map(
  ...
  tar_target(clean, prep(normalize, remove_sex = remove_sex, arraytype = arraytype)),
  tar_target(ss_clean,
             savecoldata(clean,
  ...
  tar_target(pca, pca_res(top),pattern=map(top)),
  ...
  tar_target(dmps_f , filter_dmps(dmps, p.value = 0.01, mDiff = 0.05)),      # Choose filter for DMPs
  ...
  tar_target(dmps_summary,                                                       # Summary statistics for DMPs
             summary_dmps(dmps, dir = custom_paths$dmp_folder,name=data_names),error ="continue"),
  ...
  tar_target(sumaries,summarize(dmrs = dmrs, dmps = dmps_f,path = paste0(custom_paths$results_folder,"/",data_names,"/"))),
  ...
  tar_target(ep,tibble::tibble(report_parameters,
                               # values_row=jsonlite::toJSON(vals),
